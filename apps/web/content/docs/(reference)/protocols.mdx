...
description: Open non-HTTP services over TCP or UDP with the Subvima CLI.
icon: TbBuildingTunnel
---

Subvima can tunnel raw TCP and UDP traffic so you can share things like SSH, databases, syslog, or game servers.

## TCP: expose any TCP service (SSH, DB, etc.)

```bash
Subvima tcp 22 --remote-port 2222
```
...
```bash
Subvima udp 514 --remote-port 1514
```
...
- **Port already in use**: retry without `--remote-port` to let Subvima pick one, or choose another.
