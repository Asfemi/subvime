---
title: Architecture
description: Understanding how Subvima works under the hood
icon: Cpu
---

Subvima is designed to be simple yet robust.

## High-Level Overview

The Subvima system consists of three main components:
...
graph LR
    ...
    B -->|WebSocket| C[Subvima CLI]
    C -->|HTTP| D[Localhost:3000]
```

## Connection Flow

1.  **Initialization**: When you run `Subvima 3000`, the CLI initiates a WebSocket connection to the Tunnel Server.
2.  **Authentication**: The server verifies your identity using the token provided during login or via the `--key` flag.
3.  **Tunnel Establishment**: Once authenticated, the server assigns a public URL (e.g., `https://random.tunnel.Subvima.app`) to your connection.
4.  **Request Handling**:
