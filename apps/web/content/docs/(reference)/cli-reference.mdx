---
title: CLI Reference
description: Comprehensive guide to the Subvima CLI commands and flags
icon: LuSquareTerminal
---

The Subvima CLI is your primary tool for interacting with the Subvima network. This reference covers all available commands and flags.

## Commands

### `Subvima login`

Initiates the authentication process. This command opens your default browser to the Subvima dashboard where you can authorize the CLI.
...
```bash
# Use default Subvima/config.toml
Subvima start

# Use a custom config file
Subvima start --config /path/to/config.toml
```

**Options:**

- `--config <path>`: Path to the TOML configuration file (default: `Subvima/config.toml`).

See the [Configuration Files](#configuration-files) section below for details on the config file format.

### `Subvima validate-config`
...
```bash
# Validate default Subvima/config.toml
Subvima validate-config

# Validate a custom config file
Subvima validate-config --config /path/to/config.toml
```
...
You can define multiple tunnels in a TOML configuration file and start them all at once using the `Subvima start` command.

### Config File Format

Create an `Subvima/config.toml` file in your project directory:

```toml
[global]
org = "my-team"
server_url = "wss://api.Subvima.dev/"
...
# Start all tunnels from Subvima/config.toml
Subvima start

# Start tunnels from a custom config file
Subvima start --config production.toml
```
