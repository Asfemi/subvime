...
<Callout type="info">
Password-protected tunnels are available on **paid plans** (Ray, Beam, and Pulse). [View pricing →](https://Subvima.dev/pricing)
</Callout>
...
```bash
Subvima 3000 --password mysecretpassword
```
...
You can also set a password in your `Subvima/config.toml`:
...
```bash
curl -u :mysecretpassword https://my-tunnel.tunnel.Subvima.app/api/health
```
...
curl -H "Authorization: Basic $(echo -n ':mysecretpassword' | base64)" \
  https://my-tunnel.tunnel.Subvima.app/api/health
```
