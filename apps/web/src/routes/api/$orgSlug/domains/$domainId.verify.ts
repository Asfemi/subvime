      ...
      POST: async ({ request, params }) => {
        ...
        try {
          const txtRecords = await resolveTxt(
            `_Subvima-challenge.${domain.domain}`,
          );
          ...
          if (!hasValidTxt) {
            return Response.json(
              {
                error: `TXT record verification failed. Expected "${domain.id}" at "_Subvima-challenge.${domain.domain}"`,
              },
              { status: 400 },
            );
          }
          ...
          try {
            ...
            cnameRecords.some((record) => record === "edge.Subvima.app");
          } catch (e) {
